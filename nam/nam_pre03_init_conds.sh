#!/bin/bash
# Generates initial conditions from NAM data
MPAS_DIR="$HOME/mpas/uga_mpas"
# NOT THE MODEL SOURCE DIRECTORY!

MET_DATA_DIR="$HOME/mpas/MPAS_Files/MET_DATA/NAM3k"
...
first_nam_file=$(ls -1 ./"$NAM_PREFIX":* | head -n 1)
if [ -z "$first_nam_file" ]; then
    echo "error: could not find any NAM files with prefix '$NAM_PREFIX' to determine start time."
    exit 1
fi
...
sed -i -e "s#^ *config_start_time *=.*# config_start_time = '$new_start_time:00:00'#" \
       -e "s#^ *config_met_prefix *=.*# config_met_prefix = '$NAM_PREFIX'#" namelist.init_atmosphere
