#!/bin/bash
# You MUST run the preprocessing scripts first (nam_pre*.sh)
...
# --- Main Script ---
echo "prepping atmosphere model with NAM data"
...
first_nam_file=$(ls -1 ./"$NAM_PREFIX":* | sort -V | head -n 1)
if [ -z "$first_nam_file" ]; then
    echo "error: could not find any NAM files to determine the start time."
    exit 1
fi
